...
// GetEndpointHashKey returns the ring hash key attribute of addr. If this attribute
...
// Notice: This API is EXPERIMENTAL and may be changed or removed in a
// later release.
func GetEndpointHashKey(addr resolver.Endpoint) string {
	hashKey, _ := addr.Attributes.Value(hashKeyKey).(string)
	return hashKey
}
